...
- Missing API throttling or unbounded concurrent traffic can turn auth failures, client retry loops, or presign abuse into unnecessary AWS cost and noisy telemetry.
...
- Implementation progress:
  ...
  - Replaced the inline placeholder Lambda with a bundled Node.js 24 Control API Lambda, generated Secrets Manager dev token, Lambda-layer timing-safe token check, DynamoDB/S3/secret permissions, explicit timeout/memory, HTTP API throttling, and one-week log retention.
...
- Post-merge deployment evidence:
  ...
  - PR #32 passed PR verification, was squash-merged, and the next post-merge deploy reached CDK deploy but failed because PR-010 had also replaced the API Gateway `$default` stage logical path while keeping the same physical stage name.
  - Follow-up branch `codex/pr-010-stage-migration-fix` restores CDK's default-stage construct path (`createDefaultStage: true`) and configures throttling on the existing synthesized `AWS::ApiGatewayV2::Stage` resource, avoiding creation of a second `$default` stage. Local checks passed: `pnpm --filter @agentcore-pdf-translator/infra typecheck`, `pnpm --filter @agentcore-pdf-translator/infra test`, `pnpm lint`, `pnpm ci:validate-workflow`, `pnpm cdk synth AgentCorePdfTranslator-dev-StorageStack AgentCorePdfTranslator-dev-DatabaseStack AgentCorePdfTranslator-dev-ControlApiStack -c stage=dev -c workspaceId=ci_dev -c activePriceBookVersion=ci_dev -c priceBookHumanReviewHourlyRateUsd=90 --output ../.ci/local-pr-010-stage-fix/cdk.out`, and `CDK_ASSEMBLY_DIR=.ci/local-pr-010-stage-fix/cdk.out DATA_PROTECTION_SUMMARY_PATH=.ci/local-pr-010-stage-fix/data-protection-summary.json node scripts/ci/validate-data-protection.mjs`.
  - The post-merge deploy for PR #33 updated the `$default` stage in place, then failed when setting Lambda reserved concurrency because the dev account would have fewer than AWS Lambda's minimum unreserved concurrent executions. Follow-up branch `codex/pr-010-concurrency-quota-fix` removes Lambda reserved concurrency and keeps the explicit HTTP API throttle as the deployed PR-010 concurrency/cost guard. Local checks passed: `pnpm --filter @agentcore-pdf-translator/infra typecheck`, `pnpm --filter @agentcore-pdf-translator/infra test`, `pnpm lint`, `pnpm ci:validate-workflow`, `pnpm cdk synth AgentCorePdfTranslator-dev-StorageStack AgentCorePdfTranslator-dev-DatabaseStack AgentCorePdfTranslator-dev-ControlApiStack -c stage=dev -c workspaceId=ci_dev -c activePriceBookVersion=ci_dev -c priceBookHumanReviewHourlyRateUsd=90 --output ../.ci/local-pr-010-concurrency-fix/cdk.out`, and `CDK_ASSEMBLY_DIR=.ci/local-pr-010-concurrency-fix/cdk.out DATA_PROTECTION_SUMMARY_PATH=.ci/local-pr-010-concurrency-fix/data-protection-summary.json node scripts/ci/validate-data-protection.mjs`.
