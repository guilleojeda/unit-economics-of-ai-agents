...
- Merged PR #37 (`Fix frontend app shell edge rewrite`) into `main` at merged SHA `e13a73d4e648feb37fa6a93bd28f5b3dd2f0a555`.
- Post-merge main CI run `26209939600` again failed at `Smoke test deployed protected frontend`. The new diagnostic evidence is sharper: S3 `head-object` for `index.html` in the frontend bucket succeeded with `contentLength` 9049 and `contentType` `text/html; charset=utf-8`; unauthenticated CloudFront `/` and `/api/price-books/current` were denied; authenticated CloudFront `/api/price-books/current` returned `ACTIVE`; but authenticated CloudFront `/`, `/index.html`, and `/documents` returned 404 for 59 attempts. This isolates the failure to the private S3 static origin path rather than browser Basic Auth, edge origin proof, API routing, asset publication, or root/deep-link rewrite alone.
- Third fix-forward path: created branch `codex/pr-010a-static-origin-fix` from merged `main`. This recovery must keep OAC/private S3, keep the same protected frontend smoke assertions, and fix the CloudFront-to-S3 origin permissions/diagnostics without broadening public access, adding manual AWS changes, weakening CI-only deployment, or masking missing assets with a global CloudFront custom error fallback.
- Third fix-forward plan review: HECK YES. The scope is correctly limited to the static-origin serving failure. AWS CDK documentation for `S3BucketOrigin.withOriginAccessControl` confirms `originAccessLevels` can grant OAC read and list permissions, and `LIST` is the intended way to distinguish missing objects from access failures for private S3 origins. Adding source-ARN-scoped `s3:ListBucket` for the CloudFront distribution plus stronger smoke response diagnostics is a narrow, testable fix that preserves the private OAC architecture and does not route raw PDFs or product data through the API.
- Implemented the third fix-forward by granting the CloudFront OAC source-scoped `READ` and `LIST` access to the private frontend bucket, adding an infrastructure assertion for that bucket policy, and extending frontend smoke output with cache/body fingerprints for any remaining static-origin failure.
- Third fix-forward deterministic checks passed: `node --check scripts/ci/smoke-frontend.mjs`, `pnpm --filter @agentcore-pdf-translator/infra test`, `pnpm --filter @agentcore-pdf-translator/infra typecheck`, `pnpm cdk synth ...FrontendStack`, `node scripts/ci/validate-data-protection.mjs` against `.ci/static-origin-fix/cdk.out`, `pnpm ci:validate-workflow`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `git diff --check`.
- Refactoring assessment after green checks: no additional refactor is warranted. The change is already small and localized; extracting smoke response summarization or wrapping OAC permission configuration would add indirection without reducing meaningful duplication.
- Merged PR #38 (`Fix PR-010A frontend static origin access`) into `main` at merged SHA `13493c7a902060d846bbc20ca1a3336b0ba50a76`.
- Post-merge main CI run `26211270604` again failed at `Smoke test deployed protected frontend`. The deployed stacks and frontend asset publication succeeded, and the protected CloudFront API path still returned `ACTIVE`, but authenticated CloudFront `/`, `/index.html`, and `/documents` returned S3 XML `NotFound` for 59 attempts. The smoke proved `index.html` exists in the expected frontend bucket, so the remaining failure is the CloudFront-to-S3 origin request shape rather than missing publication, browser Basic Auth, API origin proof, or app-shell routing.
- Fourth fix-forward path: create a new branch from merged `main` and scope the recovery to the static S3 origin request path. Keep the private S3 bucket, OAC, source-ARN-scoped bucket policy, viewer-request Basic Auth, and `/api/*` API behavior unchanged. Add an origin-request Lambda@Edge association only to the default static behavior so static origin requests explicitly retain the frontend bucket regional domain as the S3 origin host before CloudFront signs the S3 request. Do not attach that origin-request function to `/api/*`.
- Fourth fix-forward plan review: HECK YES. The failure is now below the app/API layer and limited to static S3 origin resolution. Explicitly pinning the static origin request host is a narrow CloudFront/S3-origin correction that does not weaken access control, does not use public buckets, does not bypass CI deployment, does not add product behavior, and keeps `/api/*` isolated. Deterministic proof must include infrastructure tests proving the default behavior has both viewer-request auth and static origin-request host pinning while the API behavior has only viewer-request auth, plus the normal full local checks before another PR/merge.
- Implemented the fourth fix-forward by adding a static-only origin-request Lambda@Edge function that pins the request S3 origin domain and `Host` header to the frontend bucket regional domain, while leaving `/api/*` with only the existing viewer-request Basic Auth/origin-proof function.
- Fourth fix-forward deterministic checks passed: `node --check scripts/ci/smoke-frontend.mjs`, `pnpm --filter @agentcore-pdf-translator/infra test`, `pnpm --filter @agentcore-pdf-translator/infra typecheck`, `pnpm cdk synth ...FrontendStack`, `node scripts/ci/validate-data-protection.mjs` against `.ci/static-origin-host/cdk.out`, `pnpm ci:validate-workflow`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `git diff --check`.
- Refactoring assessment after green checks: no broader refactor is warranted. The new origin-request function is deliberately separate from the viewer-request auth function because it must not run on `/api/*`; merging the two concerns would increase the chance of leaking static-origin assumptions into the API behavior.
