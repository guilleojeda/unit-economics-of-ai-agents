...
- V2 image, annotation, artifact, evaluation, review, and comparison persistence must commit each visible outcome as an atomic or recoverably staged record group. A V2 run, image stage, review decision, economics total, or V1/V2 comparison must not appear successful unless required image manifests, annotations, StageEvents, Artifacts/S3 metadata, EvaluationResults, ReviewDecisions where applicable, LedgerItems, and run/job status changes are committed consistently. Partial persistence must leave the affected run retryable or explicitly failed/incomplete and must block misleading artifact, economics, and comparison reads until recovery.
...
- Partial-persistence tests proving injected failures during V2 image-manifest, annotation, artifact, evaluation, ledger, review, status, or comparison-read preparation cannot leave completed image handling, accepted outcomes, artifact links, economics, or V1/V2 comparison claims assembled from incomplete record groups.
...
13. Verify a supported injected or simulated partial-persistence failure in the V2 image/annotation/evaluation/review path does not expose completed image handling, accepted outcome, artifact/evaluation success, or V1/V2 comparison/economics success from incomplete records.
...
- No completed V2 image handling, accepted outcome, artifact/evaluation success, economics, or V1/V2 comparison claim assembled from incomplete image/annotation/artifact/evaluation/review record groups.
...
- V2 persistence is atomic or recoverably staged for each visible image/review/comparison outcome; partial failures cannot produce successful image handling, accepted state, artifact/evaluation visibility, economics, or comparison claims with missing related records.
...
- Lets partial V2 persistence appear as completed image handling, accepted outcome, artifact/evaluation success, economics, or V1/V2 comparison evidence with missing manifests, annotations, StageEvents, Artifacts, EvaluationResults, ReviewDecisions, or LedgerItems.
