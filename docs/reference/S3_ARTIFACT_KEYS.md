...
Artifact buckets and objects that back registered `Artifact` records are product evidence. CDK and lifecycle configuration must not auto-delete, replace, or expire source PDFs, translated PDFs, previews, evaluations, image assets, route artifacts, skipped-stage evidence, or ledger exports that are still referenced by retained product records. Any future retention or archival behavior needs an explicit story that preserves auditability and prevents `Artifact` records from pointing to missing object evidence.
